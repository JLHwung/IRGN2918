...
This proposal requests an update of the reference glyphs for two UK-source ideographs, as listed in Table~\ref{tab:proposed-glyphs}.
...
\section{Rationale}
The current reference glyphs of U+3062B 𰘫 and U+30C68 𰱨 come from 《南明史》 (Figure~\ref{fig:nanmingshi}).
...
The suggested glyphs ⿱艹𪸔 and ⿰木聙 are also present in the printed versions of the archival documents mentioned above, published as 《明清史料》 in 1930s (Figures~\ref{fig:mqsl-1-98} and \ref{fig:mqsl-3-408}), as well as in a modern academic research article (Figure~\ref{fig:qingshiluncong}).
...
Based on the evidence, it is clear that the current glyphs for U+3062B 𰘫 and U+30C68 𰱨 are not attested in earlier historical documents. Furthermore, the suggested glyphs ⿱艹𪸔 and ⿰木聙 are more accurate representations of the original characters. Therefore, we propose to update the reference glyphs for U+3062B 𰘫 and U+30C68 𰱨 accordingly.
...
\section*{Appendix}
Table~\ref{tab:comparison} compares the person names across different sources; discrepancies are highlighted in color. Note that the characters ⿱艹炤 and ⿰木𬍐 have not yet been encoded.
\begin{table}[htbp]
  \centering
  \renewcommand{\arraystretch}{1.5}
  \renewcommand{\tabcolsep}{8pt}
  ...
  \begin{tabularx}{\textwidth}{c|c|c|c|c|c|c|c|c|c}
    ...
    \hline
  \end{tabularx}
  \caption{Comparison of person names across different sources}\label{tab:comparison}
\end{table}
